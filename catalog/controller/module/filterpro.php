...
class ControllerModuleFilterPro extends Controller {
	...
	protected function index($setting) {
		if($setting['type'] == 1) {
			...
			$seo_type = $this->config->get('filterpro_seo_type');
			...
			//manuf_seo
			if(isset($seo_type['manufacturer'])){
				$arr_man =  explode(',',$seo_type['manufacturer']['id']);
					
						
				foreach($this->data['manufacturers'] as $k => $manuf){
					if   (array_search ( ($this->data['manufacturers'][$k]['manufacturer_id']), $arr_man ) !== false){
						$this->data['manufacturers'][$k]['seo_status'] = 1;
					}
				}
			}
	
			
			if(isset($seo_type['option_value'])){
				$arr_option_value =  $seo_type['option_value']['id'];
			}else{
				
				$arr_option_value = false;
			}
			$this->data['options'] = $this->model_module_filterpro->getOptions($data, $arr_option_value);
...
if(isset($seo_type['attribute_value'])){
	$arr_attribute_value =  $seo_type['attribute_value']['id'];
}else{
	$arr_attribute_value = false;
}
			$this->data['attributes'] = $this->model_module_filterpro->getAttributes($data,$arr_attribute_value );
...
/*
			...
			$status_scroll = 0;
			if(isset($filterpro_setting['ajax_scroll'])){
				$this->document->addScript('catalog/view/javascript/ajax_product_scroll.js');
				$this->data['loadscroll_status'] = 1;
			}
			
		
			
			if(isset($filterpro_setting['ajax_more_btn'])){
				$this->document->addScript('catalog/view/javascript/ajax-product-btn.js');
				$this->data['loadmore_status'] = 1;
				if(isset($filterpro_setting['pagination_hide'])){
					$this->data['pagination_hide'] = 1;
				}
			}
		...
		if(isset($seo_type['seo'])){$seo_fil = $seo_type['seo'];}else{$seo_fil = false;}
		// echo '<pre>';
		//	var_dump($seo_type);
			$this->data['filterOptions'] = json_encode(array(
							//		'container'		=> $filterpro_setting['filterpro_container'],
								...
								//	'enable_more_ajax'	=> isset($filterpro_setting['enable_more_ajax']),
													   ));
	...
	public function getProducts() {
		...
		if(isset($this->request->get['route']) && $this->request->get['route'] == "product/special"){
					...
					$totals_manufacturers = $this->model_module_filterpro->getTotalManufacturers($data, 1);
				}

				$totals_options = $this->model_module_filterpro->getTotalOptions($data, 1);

				$totals_attributes = $this->model_module_filterpro->getTotalAttributes($data, 1);
				foreach($attribute_value as $attribute_id => $values) {
					...
					foreach($this->model_module_filterpro->getTotalAttributes($temp_data, 1) as $attribute){
						if($attribute['id'] == $attribute_id) {
				...
				if($version[2] >= 4) {
					...
					$totals_tags = $this->model_module_filterpro->getTotalTags($data, 1);
				}
				$totals_categories = $this->model_module_filterpro->getTotalCategories($data, $category_id, 1);

				$results = $this->model_module_filterpro->getProducts($data, 1);
	...
	public function getPagination(){
		...
		$pagination->limit = $limit;
		$pagination->text = $this->language->get('text_pagination');
		$json = json_encode(array('pagination' => $pagination,
								 'product_total' => $product_total));
		$this->response->setOutput($json);
	}
	*/
