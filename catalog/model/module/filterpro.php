...
class ModelModuleFilterPro extends Model {
	...
	public function getAttributes($data, $attr_type = false) {
		...
		foreach($attributes as $attribute_group_id => $attribute_group) {
			...
			//	sort($attributes[$attribute_group_id]['attribute_values'][$attribute_id]['values']);
				 ...
				 $digits = $letters = array();
				foreach($attributes[$attribute_group_id]['attribute_values'][$attribute_id]['values'] as $V){
				...
				natsort($letters);
					if($attr_type){
						...
						if   (isset($attr_type[$attribute_id])){
							
							$attributes[$attribute_group_id]['attribute_values'][$attribute_id]['seo_status'] = $attr_type[$attribute_id];	
							
						}else{
						}
					}
				$attributes[$attribute_group_id]['attribute_values'][$attribute_id]['values'] = array_merge($letters, $digits);	
	...
	public function getOptions($data, $option_type = false) {
		$sql_image = ", ov.image";
	...
	WHERE ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND od.language_id = '" . (int)$this->config->get('config_language_id') . "'  AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id =" . (int)$this->config->get('config_store_id');
		...
		if($option_type){
					$seo_status = false;
				//	$option_type_arr = explode('_',$option_type );
				//	$option_type_id = $option_type_arr[0];
				//	$option_type_value = $option_type_arr[1];
					if   (isset($option_type[$row['option_id']])){
						$seo_status = 1;
					}else{
						$seo_status = 0;
					}
		...
		if($seo_status){
			...
			if( array_search($row['option_value_id'],$option_type[$row['option_id']]) !== false ){
	
				$seo_st = $row['option_id'] . '_' . $row['option_value_id'] ;
			...
			$options[$row['option_id']]['option_values'][] = array('option_value_id' => $row['option_value_id'], 'name' => $row['name'], 'image' => $row['image'], 'seo_st' => $seo_st);
	...
	public function getTotalProducts($data, $special = 0) {
		...
		if($special) {
			...
			$sql .= "SELECT DISTINCT p.product_id, coalesce((SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1), " .
				"(SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1), " .
			...
			$sql .= "LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) " .
				...
				" LEFT JOIN " . DB_PREFIX . "product_option_value pov ON (pov.product_id=ps.product_id)" .
				" LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p2s.product_id=ps.product_id)" .
				" LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p2c.product_id=ps.product_id)";

			if ($data['attribute_value'] || $data['attr_slider']) {
				$sql .= " LEFT JOIN " . DB_PREFIX . "product_attribute p2a ON (p2a.product_id=ps.product_id)";
			}

			if($data['tags']) {
				$sql .= " LEFT JOIN " . DB_PREFIX . "product_tag p2t ON (p2t.product_id=ps.product_id)";
			}
			...
			$sql .= " AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ";
	...
	public function getTotalManufacturers($data, $special = 0) {

		$customer_group_id = $this->getCustomerGroup();

		if($special) {
			$sql = "SELECT count(*) as total, manufacturer_id FROM(";
			$sql .= "SELECT DISTINCT p.product_id, m.manufacturer_id, coalesce((SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1), " .
				"(SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1), " .
				"p.price) as realprice ";
			$sql .= "FROM " . DB_PREFIX . "product_special ps ";
			$sql .= "LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) " .
	...
	public function getTotalTags($data, $special = 0) {

		$customer_group_id = $this->getCustomerGroup();

		if($special){
			$sql = "SELECT CONCAT(`tag`, ' (', count(*), ')') as 'name', tag FROM(";
			$sql .= "SELECT DISTINCT p.product_id, tag, coalesce ((SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1), " .
				"(SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1), " .
				"p.price) as realprice ";
			$sql .= "FROM " . DB_PREFIX . "product_special ps ";
			$sql .= "LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) " .
	...
	public function getTotalCategories($data, $category_id, $special = 0) {

		$customer_group_id = $this->getCustomerGroup();

		if($special){
			$sql = "SELECT CONCAT(`name`, ' (', count(*), ')') as 'name', `category_id` FROM(";
			$sql .= "SELECT DISTINCT p.product_id, cd.name, cd.category_id, c.sort_order, coalesce((SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1), " .
				"(SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1), " .
				"p.price) as realprice ";
			$sql .= "FROM " . DB_PREFIX . "product_special ps ";
			$sql .= "LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) " .
	...
	public function getTotalAttributes($data, $special = 0) {
		...
		if($special){
			...
			$sql .= "SELECT DISTINCT p.product_id, p2a.attribute_id, p2a.text, coalesce((SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1), " .
				"(SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1), " .
				"p.price) as realprice ";
			$sql .= "FROM " . DB_PREFIX . "product_special ps ";
			$sql .= "LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) " .
	...
	public function getTotalOptions($data, $special = 0) {
		...
		if($special){
			...
			$sql .= "SELECT DISTINCT p.product_id, pov.option_value_id, coalesce((SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1), " .
				"(SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1), " .
				"p.price) as realprice ";
			$sql .= "FROM " . DB_PREFIX . "product_special ps ";
			$sql .= "LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) " .
	...
	public function getProducts($data, $special = 0) {
		$customer_group_id = $this->getCustomerGroup();
...
//		var_dump($sql);
	...
	*/
	...
	'tags' => false,
	'categories' => array($data['category_id']),
	'attr_slider' => false,
	'filter' => false,
