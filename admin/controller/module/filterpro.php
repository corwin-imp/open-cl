...
class ControllerModuleFilterPro extends Controller {
	...
	public function index() {
		...
		if(($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if(VERSION == '1.5.0') {
				...
				if(isset($this->request->post['filterpro_seo'])){
						$this->request->post['filterpro_seo_type'] = $this->seo_type($this->request->post['filterpro_seo']);
		...
		$this->data['text_hide_hash'] = $this->language->get('text_hide_hash');
		...
		$this->data['text_ajax_more_btn']  = $this->language->get('text_ajax_more_btn');
		$this->data['text_pagination_hide']  = $this->language->get('text_pagination_hide');
		$this->data['text_ajax_scroll'] 	 = $this->language->get('text_ajax_scroll');
	...
	$this->model_module_filterpro->install();
	...
	public function seo_type($data){
		...
		$id_attr = '';	 
			...
			$str = stristr($str, 'max_price'); // выводит ER@EXAMPLE.com
			$str = str_replace("[]", "", $str);
			...
			$new_arr = array();
				...
				foreach($arr as $ar){
					...
					//	$new_ar[]['type'] = ; // выводит ER@EXAMPLE.com
					//	$new_ar[]['id'] = ; // выводит ER@EXAMPLE.com
					$type_seo = stristr($ar, '=', true);
				
					$id = stristr($type_seo, '[');
					
					$id =  str_replace('[', '', $id);
				
					if(stristr($type_seo, '[', true)){
						$type_seo = stristr($type_seo, '[', true);
					}
						
					//	$id = substr(stristr($type, ']'), -1);
				
				
					if(!empty($id)){
					//	if(strlen($id) > 2){////	$id =  str_replace(']', '_', $id);//	$id_seo = $id .  substr(stristr($ar, '='), 1);	}else{//	}
						
						
					
							$id =  str_replace(']', '', $id);
							
							$id_opt = $id . '_' .  substr(stristr($ar, '='), 1);
						...
						$id_seo =  substr(stristr($ar, '='), 1);
					}
				
					if($type_seo == 'manufacturer'){
						$man_id = $id_seo;
					}
					
					if($type_seo == 'option_value'){
						$id_opt = $id . '_' .  substr(stristr($ar, '='), 1);
					}
					
					
					if($type_seo == 'attribute_value'){
						$id_at = substr(stristr($ar, '='), 1);
						$id_one =  str_replace(']', '', $id);
						
						$id_attr = $id_one . '^' .  $id_at;
					
					}
					
				
				//	if(!empty($new_ar) && isset($new_ar) && count($arr_seo) == count(array_unique($arr_seo) )){
					if(!empty($new_ar) && isset($new_ar)  ){
						foreach($new_ar as $key => $value){
							if($type_seo == $new_ar[$key]['type']){
							
							if(is_array($new_ar[$key]['id'])){
							
								
								
									$new_ar[$key]['id'][$id][] = substr(stristr($ar, '='), 1);
								
								$new_ar[$key]['id'][$id] = array_unique($new_ar[$key]['id'][$id]);
							
							}else{
								$new_ar[$key]['id'] = $new_ar[$key]['id'] . ',' . $id_seo;
							}
				...
				$str_type .= $man_id;
				if(!empty($str_type) && !empty($id_opt)){
					$str_type .= "," . $id_opt; 
				}else{
					$str_type .= $id_opt; 
				}
				
				if(!empty($str_type) && !empty($id_attr)){
					$str_type .= "," . $id_attr; 
				}else{
					$str_type .=  $id_attr; 
				}
				$arr_seo[$v['seo']] = $str_type;
